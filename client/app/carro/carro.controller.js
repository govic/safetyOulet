...
angular.module('pruebaAngularApp').controller('CarroCtrl', function($anchorScroll, $scope, $http, $localStorage, carroService, formatCurrency, $state) {
    ...
    $http.get('/api/seccion/secciones_principales').success(function(secciones) {
        if (secciones && secciones.length !== 0) {
            ...
            _.each($scope.secciones, function(sec) {
                ...
                $http.post('/api/producto/query_seccion', sec).success(function(productos_seccion) {
                    if (productos_seccion && productos_seccion.length !== 0) {
                        ...
                        console.log('seccion sin productos para mostrar');
                    }
                }).error(function(err) {
                    //error get productos
                    console.dir(err);
            ...
            console.log('sin secciones para mostrar');
        }
    }).error(function(err) {
        //error get secciones
        console.dir(err);
    ...
    //metodo para ir a pagina de compras
    $scope.goToCheckout = function() {
        //metodo anterior
        //$state.go('checkout', {accion: 'comprar'});

        //redireccion hacia pagina finalizar
        if (carroService.validaCarroCompra()) {
            $state.go('finalizar');
        } else {
            console.log('error al validar carro de compras');
        }
    };
